...
Представляет старую таблицу для хранения истории обработки изображений. Сохранена для обратной совместимости.
...
### 1. ProcessingOperation → NeuralModel (многие к одному)
...
### 2. ProcessingOperation.sessionId (логическая связь)
...
(независимая таблица для обратной совместимости)
```
...
**Таблицы:**
- `processing_history` - история обработки изображений (сохранена для обратной совместимости)
...
Работа с историей обработки (старая таблица, обратная совместимость).
...
2. **Обратная совместимость**
   - Таблица `processing_history` сохранена для поддержки старого функционала
   - Новая функциональность использует `processing_operations`
