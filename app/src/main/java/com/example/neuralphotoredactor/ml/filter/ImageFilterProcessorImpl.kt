...
class ImageFilterProcessorImpl
    @Inject
    constructor() : ImageFilterProcessor {
        override fun applyFilter(
            ...
            return try {
                    ...
                    val result =
                        when (filterType) {
                            FilterType.GAUSSIAN_BLUR ->
                                applyGaussianBlurAGSL(
                                    bitmap,
                                    intensity ?: 0.5f,
                                )
                            FilterType.NOISE_REDUCTION ->
                                applyNoiseReductionAGSL(
                                    bitmap,
                                    intensity ?: 0.5f,
                                )
                ...
                android.util.Log.e(
                    ...
                    "Ошибка применения фильтра ${filterType.name}: ${e.message}",
                    e,
                )
                e.printStackTrace()
        ...
        override fun applyFilters(
            ...
            return try {
                ...
                android.util.Log.d(
                    "ImageFilterProcessor",
                    "Применяем ${filters.size} фильтров, preview=$isPreview",
                )
                ...
                for (indexedFilter in sortedFilters.withIndex()) {
                    ...
                    val filteredResult =
                        applyFilter(
                            ...
                            intensity,
                            isPreview = false,
                        ) // isPreview=false, т.к. уже уменьшили

                    if (filteredResult == null) {
                        android.util.Log.e(
                            "ImageFilterProcessor",
                            "Фильтр $filterType (${index + 1}/${sortedFilters.size}) вернул null",
                        )
                ...
                val finalResult =
                    if (isPreview && workingBitmap != originalBitmap && workingBitmap.width != originalBitmap.width) {
                        android.util.Log.d(
                            "ImageFilterProcessor",
                            "Увеличиваем результат обратно до исходного размера",
                        )
                        val scaled =
                ...
                android.util.Log.d(
                    "ImageFilterProcessor",
                    "Все ${sortedFilters.size} фильтров применены успешно за ${duration}ms",
                )
                finalResult
            } catch (e: OutOfMemoryError) {
                android.util.Log.e(
                    "ImageFilterProcessor",
                    "OutOfMemoryError при применении фильтров: ${e.message}",
                    e,
                )
                null
            } catch (e: Exception) {
                android.util.Log.e(
                    "ImageFilterProcessor",
                    "Ошибка применения множественных фильтров: ${e.message}",
                    e,
                )
                e.printStackTrace()
                null
         ...
         */
        private fun applyFilterFallback(
            bitmap: Bitmap,
            filterType: FilterType,
            intensity: Float?,
            isPreview: Boolean,
        ...
        private fun applyGrayscaleAGSL(bitmap: Bitmap): Bitmap? {
            return try {
                ...
                android.util.Log.e(
                    "ImageFilterProcessor",
                    "Ошибка применения Grayscale AGSL: ${e.message}",
                    e,
                )
        ...
        @RequiresApi(Build.VERSION_CODES.TIRAMISU)
        private fun applySepiaAGSL(
            bitmap: Bitmap,
            intensity: Float,
        ): Bitmap? {
            return try {
                val shaderCode =
                    AGSLShaders.SEPIA_SHADER.replace(
                ...
                android.util.Log.e(
                    "ImageFilterProcessor",
                    "Ошибка применения Sepia AGSL: ${e.message}",
                    e,
                )
        ...
        @RequiresApi(Build.VERSION_CODES.TIRAMISU)
        private fun applyVignetteAGSL(
            bitmap: Bitmap,
            intensity: Float,
        ): Bitmap? {
            return try {
                val shaderCode =
                    AGSLShaders.VIGNETTE_SHADER
                        .replace(
                            "uniform float intensity;",
                            "uniform float intensity = $intensity;",
                        )
                        .replace(
                            "uniform float2 size;",
                            "uniform float2 size = float2(${bitmap.width.toFloat()}, ${bitmap.height.toFloat()});",
                        )
                val result = applyAGSLShader(bitmap, shaderCode)
                result ?: run {
                ...
                android.util.Log.e(
                    "ImageFilterProcessor",
                    "Ошибка применения Vignette AGSL: ${e.message}",
                    e,
                )
        ...
        private fun applySharpenAGSL(
            ...
            return try {
                ...
                android.util.Log.e(
                    "ImageFilterProcessor",
                    "Ошибка применения Sharpen AGSL: ${e.message}",
                    e,
                )
        ...
        @RequiresApi(Build.VERSION_CODES.TIRAMISU)
        private fun applyGaussianBlurAGSL(
            bitmap: Bitmap,
            intensity: Float,
        ): Bitmap? {
            return try {
                ...
                android.util.Log.e(
                    "ImageFilterProcessor",
                    "Ошибка применения Gaussian Blur: ${e.message}",
                    e,
                )
        ...
        @RequiresApi(Build.VERSION_CODES.TIRAMISU)
        private fun applyNoiseReductionAGSL(
            bitmap: Bitmap,
            intensity: Float,
        ): Bitmap? {
            return try {
                val shaderCode =
                    AGSLShaders.NOISE_REDUCTION_SHADER
                        .replace(
                            "uniform float intensity;",
                            "uniform float intensity = $intensity;",
                        )
                ...
                android.util.Log.e(
                    "ImageFilterProcessor",
                    "Ошибка применения Noise Reduction AGSL: ${e.message}",
                    e,
                )
        ...
        private fun applyAGSLShader(
            ...
            return try {
                ...
                android.util.Log.d(
                    "ImageFilterProcessor",
                    "Применяем AGSL шейдер, размер Bitmap: ${bitmap.width}x${bitmap.height}",
                )
                ...
                if (result == null) {
                    android.util.Log.e(
                        "ImageFilterProcessor",
                        "applyRenderEffectViaPaint вернул null",
                    )
                } else {
                    android.util.Log.d(
                        "ImageFilterProcessor",
                        "Результат создан: ${result.width}x${result.height}",
                    )
                }
                ...
                android.util.Log.e(
                    "ImageFilterProcessor",
                    "Возможно, неправильный синтаксис AGSL шейдера",
                )
                null
            } catch (e: Exception) {
                android.util.Log.e(
                    "ImageFilterProcessor",
                    "Ошибка применения AGSL: ${e.javaClass.simpleName}: ${e.message}",
                    e,
                )
                e.printStackTrace()
                null
        ...
        private fun applyRenderEffect(
            ...
            return try {
                ...
                android.util.Log.e(
                    "ImageFilterProcessor",
                    "Ошибка применения RenderEffect: ${e.message}",
                    e,
                )
                null
            }
         ...
         */
        @RequiresApi(Build.VERSION_CODES.TIRAMISU)
        private fun applyRenderEffectViaPaint(
            bitmap: Bitmap,
            effect: RenderEffect,
        ): Bitmap? {
            ...
            android.util.Log.w(
                "ImageFilterProcessor",
                "applyRenderEffectViaPaint отключен, используем fallback",
            )
            return null
        }
         ...
         */
        private fun applyGaussianBlurAlgorithmic(
            bitmap: Bitmap,
            intensity: Float,
            ...
            return try {
                ...
                android.util.Log.d(
                    "ImageFilterProcessor",
                    "Применяем Gaussian Blur: радиус=$radius, размер=${bitmap.width}x${bitmap.height}, preview=$isPreview",
                )
         ...
         */
        private fun applyMedianFilterFast(
            bitmap: Bitmap,
            kernelSize: Int,
        ): Bitmap? {
            val result = Bitmap.createBitmap(bitmap.width, bitmap.height, Bitmap.Config.ARGB_8888)
        ...
        private fun applySharpen(
            ...
            return try {
                ...
                android.util.Log.d(
                    "ImageFilterProcessor",
                    "Применяем Sharpen: сила=$strength, размер=${bitmap.width}x${bitmap.height}, preview=$isPreview",
                )
         ...
         */
        private fun applyConvolutionFilterOptimized(
            bitmap: Bitmap,
        ...
        @RequiresApi(Build.VERSION_CODES.TIRAMISU)
        private fun applyVignetteRenderEffect(
            bitmap: Bitmap,
            intensity: Float,
        ): Bitmap? {
        ...
        private fun applyVignetteAlgorithmic(
            bitmap: Bitmap,
            intensity: Float,
        ): Bitmap? {
            val result = Bitmap.createBitmap(bitmap.width, bitmap.height, Bitmap.Config.ARGB_8888)
        ...
        private fun applyConvolutionFilter(
            ...
            return try {
                ...
                android.util.Log.d(
                    "ImageFilterProcessor",
                    "Начало applyConvolutionFilter: ${bitmap.width}x${bitmap.height}, ядро: ${kernel.size}x${kernel.size}",
                )

                // Для больших изображений уменьшаем размер для производительности
                val maxDimension = 1200 // Уменьшено с 1500 для ускорения
                val scaleFactor =
                    if (maxOf(bitmap.width, bitmap.height) > maxDimension) {
                ...
                val workingBitmap =
                    if (scaleFactor < 1.0f) {
                        android.util.Log.d(
                            "ImageFilterProcessor",
                            "Уменьшаем изображение для обработки: фактор=$scaleFactor",
                        )
                        Bitmap.createScaledBitmap(
                            bitmap,
                            (bitmap.width * scaleFactor).toInt(),
                ...
                android.util.Log.d(
                    "ImageFilterProcessor",
                    "Обработка центральной области: y от $halfKernel до ${workingBitmap.height - halfKernel}",
                )
                ...
                android.util.Log.d(
                    "ImageFilterProcessor",
                    "Обработано пикселей: $processedPixels из $totalPixels",
                )
                ...
                android.util.Log.d(
                    "ImageFilterProcessor",
                    "applyConvolutionFilter завершен за ${duration}ms",
                )
                finalResult
            } catch (e: OutOfMemoryError) {
                android.util.Log.e(
                    "ImageFilterProcessor",
                    "OutOfMemoryError в applyConvolutionFilter: ${e.message}",
                    e,
                )
                null
            } catch (e: Exception) {
                android.util.Log.e(
                    "ImageFilterProcessor",
                    "Ошибка в applyConvolutionFilter: ${e.message}",
                    e,
                )
                e.printStackTrace()
                null
