 ...
 */
class AnimeGan2ImageProcessor @Inject constructor(
    ...
    suspend fun processImage(bitmap: Bitmap, filterType: FilterType): Bitmap? = withContext(Dispatchers.Default) {
        if (interpreter == null) {
            android.util.Log.e("AnimeGan2ImageProcessor", "AnimeGAN2 Interpreter не инициализирован")
        ...
        return@withContext try {
            ...
            android.util.Log.d("AnimeGan2ImageProcessor", "Input shape: ${inputShape.contentToString()}, Output shape: ${outputShape.contentToString()}")
            android.util.Log.d("AnimeGan2ImageProcessor", "Input dtype: $inputDataType, Output dtype: $outputDataType")
            android.util.Log.d("AnimeGan2ImageProcessor", "Original image: ${bitmap.width}x${bitmap.height}")
            ...
            val inputShapeDynamic = if (isCHWFormat) {
                intArrayOf(1, inputChannels, inputHeight, inputWidth)
            } else {
                intArrayOf(1, inputHeight, inputWidth, inputChannels)
            }
            
            android.util.Log.d("AnimeGan2ImageProcessor", "Resizing input tensor to: ${inputShapeDynamic.contentToString()}")
            ...
            android.util.Log.d("AnimeGan2ImageProcessor", "Updated input shape: ${updatedInputShape.contentToString()}")
            android.util.Log.d("AnimeGan2ImageProcessor", "Updated output shape: ${updatedOutputShape.contentToString()}")
            ...
            if (updatedOutputShape.size >= 4 && updatedOutputShape[2] == 1 && updatedOutputShape[3] == 1) {
                ...
                android.util.Log.d("AnimeGan2ImageProcessor", 
            ...
            when (inputDataType) {
                org.tensorflow.lite.DataType.FLOAT32 -> {
                    ...
                    android.util.Log.e("AnimeGan2ImageProcessor", "Неподдерживаемый тип входных данных: $inputDataType")
                    return@withContext null
                }
            }
            inputBuffer.rewind()
            ...
            val expectedOutputSizeBytes = if (isCHWFormat) {
                1 * 3 * outputHeight * outputWidth * 4 // float32 = 4 байта
            } else {
                1 * outputHeight * outputWidth * 3 * 4 // float32 = 4 байта
            }
            
            android.util.Log.d("AnimeGan2ImageProcessor", "Creating output buffer: $expectedOutputSizeBytes bytes for ${outputWidth}x${outputHeight} (outputHeight=$outputHeight, outputWidth=$outputWidth)")
            ...
            interpreterMutex.withLock {
                ...
                try {
                    ...
                    android.util.Log.d("AnimeGan2ImageProcessor", "Inference completed using run()")
                } catch (e: IllegalArgumentException) {
                    // Если run() выдает ошибку из-за размера тензора, используем альтернативный подход
                    android.util.Log.w("AnimeGan2ImageProcessor", 
                    ...
                    val outputShapeForTensorBuffer = if (isCHWFormat) {
                        intArrayOf(1, 3, outputHeight, outputWidth)
                    } else {
                        intArrayOf(1, outputHeight, outputWidth, 3)
                    }
                    
                    val tensorBuffer = TensorBuffer.createFixedSize(outputShapeForTensorBuffer, outputDataType)
                    ...
                    try {
                        ...
                        android.util.Log.d("AnimeGan2ImageProcessor", "Inference completed using TensorBuffer")
                        ...
                        android.util.Log.e("AnimeGan2ImageProcessor", 
            ...
            android.util.Log.d("AnimeGan2ImageProcessor", "Final output dimensions: ${finalOutputWidth}x${finalOutputHeight}")
            
            // Обрабатываем выходные данные
            outputBuffer.rewind()
            
            val outputBitmap = Bitmap.createBitmap(finalOutputWidth, finalOutputHeight, Bitmap.Config.ARGB_8888)
            ...
            when (outputDataType) {
                org.tensorflow.lite.DataType.FLOAT32 -> {
                    ...
                    android.util.Log.e("AnimeGan2ImageProcessor", "Неподдерживаемый тип выходных данных: $outputDataType")
                    return@withContext null
                }
            }
            
            outputBitmap.setPixels(outputPixels, 0, finalOutputWidth, 0, 0, finalOutputWidth, finalOutputHeight)
            
            android.util.Log.d("AnimeGan2ImageProcessor", "Обработка завершена: ${finalOutputWidth}x${finalOutputHeight}")
            outputBitmap
        } catch (e: Exception) {
            android.util.Log.e("AnimeGan2ImageProcessor", "Ошибка обработки через AnimeGAN2: ${e.message}", e)
            e.printStackTrace()
            null
        }
    }
