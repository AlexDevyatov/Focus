...
/**
 * Процессор изображений для модели Hayao (стилизация в аниме стиль).
 * 
 * Выполняет инференс модели hayao.tflite для стилизации изображений в аниме стиль.
...
class HayaoProcessor @Inject constructor(
    ...
    private suspend fun getInterpreter(): Interpreter? = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            android.util.Log.e("HayaoProcessor", "Ошибка загрузки модели: ${e.message}", e)
            null
        }
    }
    
    /**
     * Обработать изображение через модель Hayao для стилизации.
     * 
    ...
    suspend fun processImage(bitmap: Bitmap, filterType: FilterType): Bitmap? = withContext(Dispatchers.Default) {
        val interpreter = getInterpreter() ?: run {
            android.util.Log.e("HayaoProcessor", "Interpreter не инициализирован")
        ...
        return@withContext try {
            ...
            android.util.Log.d("HayaoProcessor", "Обработка изображения: ${originalWidth}x${originalHeight}")
            ...
            android.util.Log.d("HayaoProcessor", "Input shape: ${inputShape.contentToString()}, Output shape: ${outputShape.contentToString()}")
            ...
            when (inputDataType) {
                org.tensorflow.lite.DataType.FLOAT32 -> {
                    ...
                    android.util.Log.e("HayaoProcessor", "Неподдерживаемый тип входных данных: $inputDataType")
                    return@withContext null
                }
            }
            inputBuffer.rewind()
            ...
            when (outputDataType) {
                org.tensorflow.lite.DataType.FLOAT32 -> {
                    ...
                    android.util.Log.e("HayaoProcessor", "Неподдерживаемый тип выходных данных: $outputDataType")
                    return@withContext null
                }
            }
            ...
            android.util.Log.d("HayaoProcessor", "Обработка завершена: ${originalWidth}x${originalHeight}")
            finalBitmap
        } catch (e: Exception) {
            android.util.Log.e("HayaoProcessor", "Ошибка обработки: ${e.message}", e)
            e.printStackTrace()
            null
        }
    }
