...
/**
 * Процессор изображений для модели AnimeGAN Face Paint (стилизация лиц в аниме стиль).
 * 
 * Выполняет инференс модели animegan_face_paint_512_v2.tflite для стилизации лиц в аниме стиль.
 ...
 */
class AnimeGanFacePaintProcessor @Inject constructor(
    private val tfliteModelRepository: TFLiteModelRepository
) {
    
    companion object {
        private const val MODEL_INPUT_SIZE = 512
        private const val MODEL_ASSET_PATH = "animegan_face_paint_512_v2.tflite"
    }
    ...
    private suspend fun getInterpreter(): Interpreter? = withContext(Dispatchers.IO) {
        ...
        try {
            ...
            android.util.Log.e("AnimeGanFacePaintProcessor", "Ошибка загрузки модели: ${e.message}", e)
            null
        }
    }
    
    /**
     * Обработать изображение через модель AnimeGAN Face Paint для стилизации.
     * 
    ...
    suspend fun processImage(bitmap: Bitmap, filterType: FilterType): Bitmap? = withContext(Dispatchers.Default) {
        val interpreter = getInterpreter() ?: run {
            android.util.Log.e("AnimeGanFacePaintProcessor", "Interpreter не инициализирован")
        ...
        return@withContext try {
            ...
            android.util.Log.d("AnimeGanFacePaintProcessor", "Обработка изображения: ${originalWidth}x${originalHeight}")
            ...
            android.util.Log.d("AnimeGanFacePaintProcessor", "Input shape: ${inputShape.contentToString()}, Output shape: ${outputShape.contentToString()}")
            ...
            when (inputDataType) {
                org.tensorflow.lite.DataType.FLOAT32 -> {
                    ...
                    android.util.Log.e("AnimeGanFacePaintProcessor", "Неподдерживаемый тип входных данных: $inputDataType")
                    return@withContext null
                }
            }
            inputBuffer.rewind()
            ...
            when (outputDataType) {
                org.tensorflow.lite.DataType.FLOAT32 -> {
                    ...
                    android.util.Log.e("AnimeGanFacePaintProcessor", "Неподдерживаемый тип выходных данных: $outputDataType")
                    return@withContext null
                }
            }
            ...
            android.util.Log.d("AnimeGanFacePaintProcessor", "Обработка завершена: ${originalWidth}x${originalHeight}")
            finalBitmap
        } catch (e: Exception) {
            android.util.Log.e("AnimeGanFacePaintProcessor", "Ошибка обработки: ${e.message}", e)
            e.printStackTrace()
            null
        }
    }
