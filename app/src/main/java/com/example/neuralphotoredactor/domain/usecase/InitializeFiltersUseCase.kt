 ...
 */
class InitializeFiltersUseCase
    @Inject
    constructor(
            ...
            withContext(Dispatchers.IO) {
                try {
                    ...
                    if (filtersList.isNotEmpty()) {
                        android.util.Log.d(
                            "InitializeFilters",
                            "Фильтры уже инициализированы: ${filtersList.size} фильтров",
                        )
                    ...
                    for (filterInfo in filtersToInitialize) {
                        try {
                            val modelId =
                                filterInfo.modelName?.let { modelName ->
                            ...
                            android.util.Log.d(
                                "InitializeFilters",
                                "Фильтр добавлен: ${filterInfo.name} (modelId: $modelId)",
                            )
                        } catch (e: Exception) {
                            android.util.Log.e(
                                "InitializeFilters",
                                "Ошибка добавления фильтра ${filterInfo.name}: ${e.message}",
                                e,
                            )
                        }
                    }
                    ...
                    android.util.Log.d(
                        "InitializeFilters",
                        "Инициализация завершена: $successCount из ${filtersToInitialize.size} фильтров",
                    )
                    successCount == filtersToInitialize.size
                } catch (e: Exception) {
                    android.util.Log.e(
                        "InitializeFilters",
                        "Ошибка инициализации фильтров: ${e.message}",
                        e,
                    )
                    false
                }
