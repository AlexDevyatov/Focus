 ...
 * если их там еще нет.
 */
class InitializeNeuralModelsUseCase
    @Inject
    constructor(
            ...
            withContext(Dispatchers.IO) {
                try {
                    ...
                    android.util.Log.d(
                        ...
                        "В БД уже есть ${existingModels.size} моделей",
                    )
                    ...
                    val assetFiles =
                        try {
                            ...
                            android.util.Log.e(
                                "InitializeNeuralModels",
                                "Ошибка сканирования assets: ${e.message}",
                                e,
                            )
                            emptyList()
                        }
                    ...
                    android.util.Log.d(
                        "InitializeNeuralModels",
                        "Найдено ${assetFiles.size} .tflite файлов в assets",
                    )
                    ...
                    for (assetFile in assetFiles) {
                        ...
                        if (existingFilePaths.contains(filePath)) {
                            android.util.Log.d(
                                "InitializeNeuralModels",
                                "Модель уже есть в БД: $assetFile",
                            )
                            skippedCount++
                            continue
                        }
                        ...
                        if (modelInfo == null) {
                            android.util.Log.w(
                                "InitializeNeuralModels",
                                "Не удалось определить метаданные для: $assetFile",
                            )
                            continue
                        }
                        ...
                        try {
                            ...
                            if (fileSize > 0) {
                                ...
                                android.util.Log.d(
                                    "InitializeNeuralModels",
                                    "Модель добавлена: ${modelInfo.name} ($assetFile)",
                                )
                            } else {
                                android.util.Log.w(
                                    "InitializeNeuralModels",
                                    "Не удалось получить размер файла: $assetFile",
                                )
                            }
                        } catch (e: Exception) {
                            android.util.Log.e(
                                "InitializeNeuralModels",
                                "Ошибка добавления модели $assetFile: ${e.message}",
                                e,
                            )
                        }
                    }
                    ...
                    android.util.Log.d(
                        "InitializeNeuralModels",
                        "Инициализация завершена: добавлено $successCount, пропущено $skippedCount из ${assetFiles.size} моделей",
                    )
                    true
                } catch (e: Exception) {
                    android.util.Log.e(
                        "InitializeNeuralModels",
                        "Ошибка инициализации моделей: ${e.message}",
                        e,
                    )
                    false
                }
        ...
        private fun getModelInfo(assetFileName: String): ModelInfo? {
            return when (assetFileName.lowercase()) {
                ...
                "animegan_face_paint_512_v2.tflite" ->
                    ModelInfo(
                ...
                "splitternet_midd_model.tflite" ->
                    ...
                    android.util.Log.w(
                        "InitializeNeuralModels",
                        "Неизвестная модель, используем дефолтные метаданные: $assetFileName",
                    )
                    ModelInfo(
                        name =
                            assetFileName.removeSuffix(
                                ".tflite",
        ...
        private fun getAssetFileSize(assetPath: String): Long {
            return try {
                ...
                android.util.Log.e(
                    "InitializeNeuralModels",
                    "Ошибка получения размера файла $assetPath: ${e.message}",
                    e,
                )
                0L
            }
