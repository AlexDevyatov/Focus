...
object DatabaseModule {
    ...
    fun provideAppDatabase(
        ...
        if (oldDbFile.exists()) {
            try {
                ...
                android.util.Log.d(
                    "DatabaseModule",
                    "Удалена старая база данных: ai_image_editor_db",
                )
            } catch (e: Exception) {
    ...
    @Provides
    @Singleton
    fun provideProcessingOperationDao(database: AppDatabase): ProcessingOperationDao {
    ...
    @Provides
    @Singleton
    fun provideNeuralModelDao(database: AppDatabase): NeuralModelDao {
