 ...
 */
class ProcessingOperationRepositoryImpl
    @Inject
    constructor(
        ...
        override suspend fun getOperationById(id: Long): ProcessingOperation? =
            withContext(Dispatchers.IO) {
                val entity = processingOperationDao.getOperationById(id)
                entity?.let { ProcessingOperationMapper.toDomain(it) }
            }

        override suspend fun getLastOperationBySessionId(sessionId: Long): ProcessingOperation? =
            withContext(Dispatchers.IO) {
                val entity = processingOperationDao.getLastOperationBySessionId(sessionId)
                entity?.let { ProcessingOperationMapper.toDomain(it) }
            }

        override suspend fun addOperation(operation: ProcessingOperation): Long =
            withContext(Dispatchers.IO) {
                val entity = ProcessingOperationMapper.toEntity(operation)
                processingOperationDao.insert(entity)
            }

        override suspend fun deleteOperation(operation: ProcessingOperation) =
            withContext(Dispatchers.IO) {
                val entity = ProcessingOperationMapper.toEntity(operation)
                processingOperationDao.delete(entity)
            }

        override suspend fun deleteOperationsBySessionId(sessionId: Long) =
            withContext(Dispatchers.IO) {
