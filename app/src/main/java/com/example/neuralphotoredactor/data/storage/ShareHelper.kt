...
class ShareHelper
    ...
    constructor(
        ...
        private fun isAppInstalled(packageName: String): Boolean {
            ...
            val installedViaPackageInfo =
                try {
                    ...
                    android.util.Log.d(
                        "ShareHelper",
                        "Приложение $packageName найдено через getPackageInfo",
                    )
                    true
                } catch (e: PackageManager.NameNotFoundException) {
                    android.util.Log.d(
                        "ShareHelper",
                        "Приложение $packageName не найдено через getPackageInfo",
                    )
                    false
                } catch (e: Exception) {
                    android.util.Log.e(
                        "ShareHelper",
                        "Ошибка проверки установки приложения $packageName через getPackageInfo: ${e.message}",
                        e,
                    )
                    false
                }
            ...
            return try {
                ...
                android.util.Log.d(
                    "ShareHelper",
                    "Приложение $packageName найдено через queryIntentActivities: $found",
                )
                found
            } catch (e: Exception) {
                android.util.Log.e(
                    "ShareHelper",
                    "Ошибка проверки установки приложения $packageName через queryIntentActivities: ${e.message}",
                    e,
                )
                false
            }
         ...
         */
        suspend fun createVkShareIntent(bitmap: Bitmap): Intent? =
         ...
         * @return Intent для запуска или null, если приложение не установлено
         */
        suspend fun createTelegramShareIntent(bitmap: Bitmap): Intent? =
            withContext(Dispatchers.IO) {
         ...
         */
        private suspend fun createShareableUri(bitmap: Bitmap): Uri? =
            withContext(Dispatchers.IO) {
                try {
                    ...
                    android.util.Log.e(
                        "ShareHelper",
                        "Ошибка создания URI для шаринга: ${e.message}",
                        e,
                    )
                    null
                }
