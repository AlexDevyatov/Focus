...
class ProcessingRepositoryImpl
    ...
    constructor(
        ...
        override suspend fun processImage(
            ...
            withContext(Dispatchers.IO) {
                ...
                try {
                    ...
                    val processedUri =
                        imageStorage.saveBitmap(processedBitmap, fileName)
                            ?: return@withContext null
                    ...
                    val filterId =
                        findFilterIdByName(filterType.name)
                    ...
                    android.util.Log.d(
                        "ProcessingRepository",
                        "Создана операция: filterId=${operationEntity.filterId}, historyId=${operationEntity.historyId}",
                    )

                    // Сохраняем в транзакции
                    val savedHistoryId =
                        appDatabase.saveHistoryWithOperations(
                            historyEntity,
                            listOf(operationEntity),
                        )
                    android.util.Log.d(
                        "ProcessingRepository",
                        "Транзакция завершена, historyId=$savedHistoryId",
                    )
                    ...
                    android.util.Log.e(
                        "ProcessingRepository",
                        "Ошибка обработки изображения: ${e.message}",
                        e,
                    )
                    null
                }
        ...
        override suspend fun deleteProcessingResult(result: ProcessingResult) =
            withContext(Dispatchers.IO) {
                ...
                val entityToDelete =
                    processingHistoryDao.findByUriAndTimestamp(
            ...
            withContext(Dispatchers.Default) {
                try {
                    ...
                    if (bitmap.isRecycled) {
                        android.util.Log.e(
                            "ProcessingRepository",
                            "Bitmap переработан, невозможно применить фильтр",
                        )
                        return@withContext null
                    }
                    ...
                    android.util.Log.d(
                        "ProcessingRepository",
                        "Применяем фильтр $filterType к Bitmap ${bitmap.width}x${bitmap.height}",
                    )
                    ...
                    val result =
                        when (filterType) {
                            ...
                            -> {
                                ...
                                imageFilterProcessor.applyFilter(
                                    bitmap,
                                    filterType,
                                    intensity,
                                    isPreview = true,
                            ...
                            FilterType.STYLE_TRANSFER -> {
                                ...
                                val scaledBitmap =
                                    if (bitmap.width > maxPreviewDimension || bitmap.height > maxPreviewDimension) {
                                        ...
                                        val scaledWidth =
                                            (bitmap.width * scale).toInt().coerceAtLeast(
                                                1,
                                            )
                                        val scaledHeight =
                                            (bitmap.height * scale).toInt().coerceAtLeast(
                                                1,
                                            )
                                        android.util.Log.d(
                                            "ProcessingRepository",
                    ...
                    if (result != null) {
                        android.util.Log.d(
                            "ProcessingRepository",
                            "Фильтр применен успешно: ${result.width}x${result.height}",
                        )
                    } else {
                    ...
                    android.util.Log.e(
                        "ProcessingRepository",
                        "Ошибка применения фильтра: ${e.message}",
                        e,
                    )
                    null
                }
            ...
            withContext(Dispatchers.Default) {
                try {
                    if (bitmap.isRecycled) {
                        android.util.Log.e(
                            "ProcessingRepository",
                            "Bitmap переработан, невозможно применить фильтры",
                        )
                        return@withContext null
                    }
                    ...
                    android.util.Log.d(
                        "ProcessingRepository",
                        "Применяем ${filters.size} фильтров к Bitmap ${bitmap.width}x${bitmap.height}",
                    )

                    // Разделяем фильтры на обычные и нейросетевые
                    val regularFilters =
                        filters.filter { (filterType, _) ->
                            filterType in
                                listOf(
                                    FilterType.GAUSSIAN_BLUR,
                                    FilterType.NOISE_REDUCTION,
                    ...
                    for ((filterType, _) in neuralFilters) {
                        val processed =
                            when (filterType) {
                                FilterType.DENOISE -> {
                                    // Используем SplitterNetImageProcessor для удаления шумов
                                    splitterNetImageProcessor.processImage(
                                        workingBitmap,
                                        filterType,
                                    )
                                }
                                FilterType.STYLE_TRANSFER -> {
                                    ...
                                    val maxPreviewDimension = 512 // Меньший размер для быстрого предпросмотра стилизации
                                    val scaledBitmap =
                                        if (workingBitmap.width > maxPreviewDimension || workingBitmap.height > maxPreviewDimension) {
                                            val scale =
                                                minOf(
                                            ...
                                            val scaledWidth =
                                                (workingBitmap.width * scale).toInt().coerceAtLeast(
                                                    1,
                                                )
                                            val scaledHeight =
                                                (workingBitmap.height * scale).toInt().coerceAtLeast(
                                                    1,
                                                )
                                            android.util.Log.d(
                                                "ProcessingRepository",
                        ...
                        if (processed != null) {
                            ...
                            android.util.Log.e(
                                "ProcessingRepository",
                                "Нейросетевой фильтр $filterType вернул null",
                            )
                    ...
                    if (result != null) {
                        android.util.Log.d(
                            "ProcessingRepository",
                            "Фильтры применены успешно: ${result.width}x${result.height}",
                        )
                    } else {
                        android.util.Log.e(
                            "ProcessingRepository",
                            "Применение фильтров вернуло null",
                        )
                    }
                    ...
                    android.util.Log.e(
                        "ProcessingRepository",
                        "Ошибка применения фильтров: ${e.message}",
                        e,
                    )
                    null
                }
        ...
        override suspend fun processImageWithFilters(
            ...
            withContext(Dispatchers.IO) {
                ...
                try {
                    ...
                    android.util.Log.d(
                        "ProcessingRepository",
                        "Обработка изображения с ${filters.size} фильтрами",
                    )
                    ...
                    android.util.Log.d(
                        "ProcessingRepository",
                        "Сохранение обработанного изображения: $fileName",
                    )
                    val processedUri =
                        imageStorage.saveBitmap(processedBitmap, fileName)
                            ?: return@withContext null
                    ...
                    android.util.Log.d(
                        "ProcessingRepository",
                        "Изображение сохранено: $processedUri",
                    )
                    ...
                    val operations =
                        filters.mapIndexed { index, (filterType, intensity) ->
                            val filterId =
                                findFilterIdByName(filterType.name)
                                    ?: throw IllegalStateException("Фильтр ${filterType.name} не найден в базе данных")
                            val operation =
                    ...
                    android.util.Log.d(
                        "ProcessingRepository",
                        "Создано операций: ${operations.size}",
                    )
                    if (operations.isEmpty()) {
                        ...
                        operations.forEachIndexed { index, op ->
                            android.util.Log.d(
                                "ProcessingRepository",
                                "Операция $index: filterId=${op.filterId}, sequenceNumber=${op.sequenceNumber}, historyId=${op.historyId}",
                            )
                        }
                    }
                    ...
                    android.util.Log.d(
                        "ProcessingRepository",
                        "Транзакция завершена, historyId=$savedHistoryId, результат сохранен в базу данных",
                    )
                    ...
                    android.util.Log.e(
                        "ProcessingRepository",
                        "OutOfMemoryError при обработке: ${e.message}",
                        e,
                    )
                    ...
                    android.util.Log.e(
                        "ProcessingRepository",
                        "Ошибка обработки с множественными фильтрами: ${e.message}",
                        e,
                    )
                    // Освобождаем память при ошибке
                    processedBitmap?.let { if (!it.isRecycled) it.recycle() }
        ...
        override suspend fun loadBitmapFromUri(uri: android.net.Uri): Bitmap? =
            withContext(Dispatchers.IO) {
                try {
                    val inputStream =
                    ...
                    inputStream.use {
                        ...
                        if (bitmap == null) {
                            android.util.Log.e(
                                "ProcessingRepository",
                                "BitmapFactory.decodeStream вернул null для URI: $uri",
                            )
                        } else {
                            android.util.Log.d(
                                "ProcessingRepository",
                                "Bitmap загружен: ${bitmap.width}x${bitmap.height}",
                            )
                        }
                        bitmap
                    }
                    ...
                    android.util.Log.e(
                        "ProcessingRepository",
                        "Ошибка загрузки Bitmap: ${e.message}",
                        e,
                    )
                    null
                }
        ...
        private suspend fun findFilterIdByName(name: String): Long? {
            return try {
                ...
                android.util.Log.e(
                    "ProcessingRepository",
                    "Ошибка поиска фильтра для $name: ${e.message}",
                    e,
                )
                null
            }
        ...
        private suspend fun processStyleTransfer(
            ...
            withContext(Dispatchers.Default) {
                when (modelName) {
                    "AnimeGAN Face Paint" ->
                        animeGanFacePaintProcessor.processImage(
                            bitmap,
                            FilterType.STYLE_TRANSFER,
                        )
                    "CelebA Distill" ->
                        celebaDistillProcessor.processImage(
                            bitmap,
                            FilterType.STYLE_TRANSFER,
                        )
                    "Hayao" -> hayaoProcessor.processImage(bitmap, FilterType.STYLE_TRANSFER)
                    "AnimeGAN2 Paprika", null ->
                        animeGan2ImageProcessor.processImage(
                            bitmap,
                            FilterType.STYLE_TRANSFER,
                        )
                    else -> {
                        android.util.Log.w(
                            "ProcessingRepository",
                            "Неизвестная модель стилизации: $modelName, используем AnimeGAN2",
                        )
            ...
            withContext(Dispatchers.Default) {
                try {
                    if (bitmap.isRecycled) {
                        android.util.Log.e(
                            "ProcessingRepository",
                            "Bitmap переработан, невозможно применить редактирование",
                        )
                        return@withContext null
                    }
                    ...
                    android.util.Log.d(
                        "ProcessingRepository",
                        "Применяем редактирование $editType к Bitmap ${bitmap.width}x${bitmap.height}",
                    )
                    ...
                    if (result != null) {
                        android.util.Log.d(
                            "ProcessingRepository",
                            "Редактирование применено успешно: ${result.width}x${result.height}",
                        )
                    } else {
                    ...
                    android.util.Log.e(
                        "ProcessingRepository",
                        "Ошибка применения редактирования: ${e.message}",
                        e,
                    )
                    null
                }
        ...
        override suspend fun saveEditedImageToGallery(
            ...
            withContext(Dispatchers.IO) {
                try {
                    ...
                    android.util.Log.d(
                        "ProcessingRepository",
                        "Сохранение обработанного изображения в галерею и processed: $finalFileName",
                    )
                    ...
                    if (galleryUri != null) {
                        android.util.Log.d(
                            "ProcessingRepository",
                            "Изображение сохранено в галерею: $galleryUri",
                        )
                    } else {
                        android.util.Log.e(
                            "ProcessingRepository",
                    ...
                    if (processedUri != null) {
                        android.util.Log.d(
                            "ProcessingRepository",
                            "Изображение сохранено в processed: $processedUri",
                        )
                                            ...
                                            if (editPair != null) {
                                                ...
                                                if (editTypeName != null) {
                                                    ...
                                                    if (filterId != null) {
                                                        ...
                                                        android.util.Log.w(
                                                            "ProcessingRepository",
                                                            "Фильтр не найден для операции редактирования: $editTypeName",
                                                        )
                                                    }
                                                }
                                            }
                                        ...
                                        if (editSettingsMap.containsKey(settingKey)) {
                                            ...
                                            if (editTypeName != null) {
                                                ...
                                                if (filterId != null) {
                                                    ...
                                                    android.util.Log.w(
                                                        "ProcessingRepository",
                                                        "Фильтр не найден для настройки: $editTypeName",
                                                    )
                                                }
                                            }
                                        }
                        ...
                        // Сохраняем в транзакции
                        val operationEntities =
                            operations.map {
                                ProcessingOperationMapper.toEntity(
                                    it,
                                )
                            }
                        android.util.Log.d(
                            "ProcessingRepository",
                            "Создано операций для сохранения: ${operationEntities.size}",
                        )
                        if (operationEntities.isEmpty()) {
                            android.util.Log.e(
                                "ProcessingRepository",
                                "ОШИБКА: Список операций пуст!",
                            )
                        } else {
                            operationEntities.forEachIndexed { index, op ->
                                android.util.Log.d(
                                    "ProcessingRepository",
                                    "Операция $index: filterId=${op.filterId}, sequenceNumber=${op.sequenceNumber}",
                                )
                            }
                        }
                        val savedHistoryId =
                        ...
                        android.util.Log.e(
                            "ProcessingRepository",
                            "Не удалось сохранить изображение в processed",
                        )
                    }
