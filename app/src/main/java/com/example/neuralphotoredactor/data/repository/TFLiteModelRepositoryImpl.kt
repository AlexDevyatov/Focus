 ...
 */
@Singleton
class TFLiteModelRepositoryImpl
    @Inject
    constructor(
        ...
        override suspend fun getInterpreterForModel(model: NeuralModel): Interpreter? =
        ...
        override suspend fun getInterpreterForModelId(modelId: Long): Interpreter? =
            withContext(Dispatchers.IO) {
                val model = neuralModelRepository.getModelById(modelId) ?: return@withContext null
                getInterpreterForModel(model)
            }

        override suspend fun loadModelFromPath(modelPath: String): Interpreter? =
            withContext(Dispatchers.IO) {
                mutex.withLock {
                    try {
                        ...
                        android.util.Log.e(
                            "TFLiteModelRepository",
                            "Ошибка загрузки модели из $modelPath: ${e.message}",
                            e,
                        )
                        null
                    }
        ...
        override suspend fun loadModelFromAssets(assetPath: String): Interpreter? =
            withContext(Dispatchers.IO) {
                mutex.withLock {
                    try {
                        ...
                        android.util.Log.d(
                            "TFLiteModelRepository",
                            "Модель загружена из assets: $assetPath",
                        )
                        interpreter
                    } catch (e: Exception) {
                        android.util.Log.e(
                            "TFLiteModelRepository",
                            "Ошибка загрузки модели из assets/$assetPath: ${e.message}",
                            e,
                        )
                        null
                    }
            ...
            interpreterCache.entries.removeIf { (_, cachedInterpreter) ->
                if (cachedInterpreter === interpreter) {
                    try {
                        ...
                        android.util.Log.e(
                            "TFLiteModelRepository",
                            "Ошибка освобождения Interpreter: ${e.message}",
                            e,
                        )
                        false
                    }
