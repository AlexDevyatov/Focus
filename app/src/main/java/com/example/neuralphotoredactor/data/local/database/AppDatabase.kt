    ...
    suspend fun saveHistoryWithOperations(
        ...
        val historyId =
            withTransaction {
                android.util.Log.d(
                    "AppDatabase",
                    "Начало транзакции: операций для сохранения: ${operations.size}",
                )
                ...
                if (operations.isNotEmpty()) {
                    ...
                    android.util.Log.d(
                        "AppDatabase",
                        "Сохранение ${operationsWithHistoryId.size} операций с historyId=$id",
                    )
                    ...
                    android.util.Log.d(
                        "AppDatabase",
                        "insertAll вызван для ${operationsWithHistoryId.size} операций",
                    )
                } else {
                    android.util.Log.w(
                        "AppDatabase",
                        "Список операций пуст, операции не сохраняются",
                    )
                }
        ...
        if (operations.isNotEmpty()) {
            val savedOperations =
                processingOperationDao().getOperationsByHistoryIdSuspend(
                    historyId,
                )
            android.util.Log.d(
                "AppDatabase",
                "Проверка после транзакции: сохранено операций в БД: ${savedOperations.size}",
            )
            if (savedOperations.size != operations.size) {
                android.util.Log.e(
                    "AppDatabase",
                    "ОШИБКА: Ожидалось ${operations.size} операций, но сохранено ${savedOperations.size}",
                )
            } else {
                android.util.Log.d(
                    "AppDatabase",
                    "Операции успешно сохранены: ${savedOperations.size}",
                )
            }
        }
