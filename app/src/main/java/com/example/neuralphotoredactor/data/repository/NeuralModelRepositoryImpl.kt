 ...
 */
class NeuralModelRepositoryImpl @Inject constructor(
    ...
    override suspend fun getModelById(id: Long): NeuralModel? = withContext(Dispatchers.IO) {
        val entity = neuralModelDao.getModelById(id)
        entity?.let { NeuralModelMapper.toDomain(it) }
    }
    
    override suspend fun getModelByName(name: String): NeuralModel? = withContext(Dispatchers.IO) {
        val entity = neuralModelDao.getModelByName(name)
        entity?.let { NeuralModelMapper.toDomain(it) }
    }
    
    override suspend fun addModel(model: NeuralModel): Long = withContext(Dispatchers.IO) {
        val entity = NeuralModelMapper.toEntity(model)
        neuralModelDao.insert(entity)
    }
    
    override suspend fun updateModel(model: NeuralModel) = withContext(Dispatchers.IO) {
        val entity = NeuralModelMapper.toEntity(model)
        neuralModelDao.update(entity)
    }
    
    override suspend fun deleteModel(model: NeuralModel) = withContext(Dispatchers.IO) {
        val entity = NeuralModelMapper.toEntity(model)
        neuralModelDao.delete(entity)
    }
    
    override suspend fun setModelActive(id: Long, isActive: Boolean) = withContext(Dispatchers.IO) {
