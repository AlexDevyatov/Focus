...
interface ProcessingOperationDao {
     ...
     */
    @Query("SELECT * FROM processing_operations WHERE sessionId = :sessionId ORDER BY sequenceNumber ASC")
     ...
     */
    @Query("SELECT * FROM processing_operations WHERE sessionId = :sessionId ORDER BY sequenceNumber DESC LIMIT 1")
     ...
     */
    @Query("SELECT COALESCE(MAX(sequenceNumber), 0) FROM processing_operations WHERE sessionId = :sessionId")
